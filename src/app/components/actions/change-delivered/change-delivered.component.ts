...
export class ChangeDeliveredComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this._subscriptions.push(this._statusBarService.obtenerFactura().subscribe(estado => {
      if (estado) {
        this.changeData.isButtonFactura = true;
        this.sendPrint();
        this._statusBarService.setEmitirFactura(false);
      }
    }));
  ...
  intro() {
    ...
    if (this.currentDocument.paymentDetails != undefined) {
      this.currentDocument.paymentDetails.forEach(pD => {
        if (pD.method.type = 72) {
          pagadoPromo = pD.primaryCurrencyTakenAmount;
        }
      });
    }
    if (!this._signalRTMEService.getStatusConnection()) {
      ...
      Promise.resolve(this._signalRTMEService.startInitializationProcess()).then(response => {
        ...
        if ((this.changeData.total - pagadoPromo) > parseFloat(this.value)
          ...
          if ((this.changeData.total - pagadoPromo) > +this.value && this.ComprobarLimpiarLineasArt(this._documentInternalService.currentDocument)) {
            this._confirmActionSvc.promptActionConfirm(
            ...
            this.pendiente = (this.changeData.total - +this.value - pagadoPromo) < 0 ? 0 : (this.changeData.total - +this.value - pagadoPromo);
            if ((this.changeData.total - pagadoPromo) > +this.value) {
  ...
  requestInvoiceClick(emitirFactura: Boolean) {
    ...
    if (emitirFactura && this.currentDocument.customer.id.substring(5) === '00000') {
      ...
      this._statusBarService.setEmitirFactura(emitirFactura);
      return;
